      ...
      <div className="grid h-full grid-cols-1 lg:grid-cols-[280px_minmax(360px,1fr)_280px]">
        ...
        <section className="flex min-h-0 flex-col bg-[#f7f7f7]"><div className="flex h-[66px] shrink-0 items-center justify-between border-b border-zinc-200 bg-white px-5"><div><h4 className="font-bold">{selectedCase.nickname || selectedCase.name}</h4><span className="text-xs font-medium text-emerald-600">● 綠色處方進行中</span></div><button className="rounded-lg border border-zinc-200 px-3 py-2 text-xs font-bold">查看個案</button></div>
          <div className="min-h-0 flex-1 overflow-y-auto p-5">
            <AutoAvatarMessage><p className="font-bold">親愛的會員您好！歡迎來到【示範診所】線上健康諮詢頻道 🌿</p><p className="mt-3">本頻道支援「綠色處方燈」服務。我們致力於透過非藥物的健康生活型態（運動、營養、舒眠、減壓）引導您改善健康。請填寫生活型態問卷告訴我們您最在意的生活面向，專家團隊將為您分析並開立專屬綠色處方！</p></AutoAvatarMessage>
            <AutoAvatarMessage><div className="rounded-2xl border border-emerald-300 bg-white p-4"><div className="flex items-center gap-3"><span className="text-3xl">📋</span><div><p className="font-black">綠色家庭醫學推動計畫</p><p className="text-xs font-bold text-emerald-600">台灣家庭醫學醫學會 × WaCare 共同推動</p></div></div><p className="mt-4 text-zinc-600">請前往活動頁完成「步驟一：同意書簽署」與「步驟二：生活型態問卷填寫」，完成後診所將為您調配專屬處方！</p><a href="https://wacare.app/green-rx/event" target="_blank" rel="noreferrer" className="mt-4 flex items-center justify-between rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 font-bold text-sky-700"><span>🌿 綠色處方活動專屬頁<br /><span className="font-medium underline">https://wacare.app/green-rx/event</span></span><ExternalLink className="h-5 w-5" /></a></div></AutoAvatarMessage>
          </div>
